...
import { getProfile, signIn } from "@/apis/auth.api";
...
const login = async () => {
    loading.value = true;
    const res = await signIn(form.value.email, form.value.password, false);
    loading.value = false;
                    ...
                    <form :class="cn('flex flex-col gap-6')" @submit.prevent="login">
                        <FieldGroup>
                            ...
                            <Field>
                                ...
                                <Input v-model="form.email" id="email" type="email" placeholder="your@gmail.com" />
